...
export default function SilverCompetitionPage() {
  ...
  // Define the total prize pool for this specific silver competition page
  const MAX_COMPETITION_PARTICIPANTS = 100; // Fixed max participants for all competitions
  const SILVER_STARTING_CAPITAL = 25000; // From image
  ...
  useEffect(() => {
    ...
    const fetchCompetitionData = async () => {
      setLoading(true);
      try {
        const leagueData = await LeagueService.getLeague("silver");
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      if (!league) {
        console.log("Interval running, but league is null. Skipping status update.");
        return; // Should ideally not happen if useEffect dependency is on league
      }
      console.log("Interval tick:", new Date().toISOString());
      console.log("League AEST times:", {
        registration_open_aest: league.registration_open_aest,
        registration_close_aest: league.registration_close_aest,
        start_time_aest: league.start_time_aest,
        end_time_aest: league.end_time_aest
      });
      ...
      console.log("Calculated Countdown Text:", countdown);
    }, 1000);
          ...
          <>
            <div className="text-center mb-8">
              {league.competition_type && (
                <Badge className="mb-4 bg-primary text-primary-foreground">{league.name} - {league.competition_type?.toUpperCase()}</Badge>
              )}
              <h1 className="text-4xl font-bold text-foreground mb-4">{league.name} Trading Competition</h1>
              <p className="text-xl text-muted-foreground">Advance from Bronze and compete for bigger prizes!</p>
            </div>
              ...
              <Card className="card-modern">
                ...
                <CardContent className="space-y-4">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-primary">${league.entry_fee}</span>
                  </div>
                  <div className="flex justify-between">
            ...
            <Card className="mb-8 card-modern">
              ...
              <CardContent>
                <ul className="space-y-2 text-muted-foreground">
                  <li>• Start with $25,000 virtual capital</li> {/* Updated rule */}
                  <li>• Trade US stocks, ETFs, and Forex</li> {/* Updated rule */}
                  <li>• No options or derivatives allowed</li>
                  <li>• Maximum position size: 15% of portfolio</li> {/* Updated rule */}
                      ...
                      <tbody className="divide-y divide-border">
                        {leaderboard.map((portfolio, index) => (
                          <tr key={portfolio.user_id}>
                            ...
                            <td className={`px-4 py-2 whitespace-nowrap text-sm ${portfolio.total_equity - SILVER_STARTING_CAPITAL >= 0 ? "text-green-500" : "text-red-500"}`}>
                              {((portfolio.total_equity - SILVER_STARTING_CAPITAL) / SILVER_STARTING_CAPITAL * 100).toFixed(2)}%
                            </td>
