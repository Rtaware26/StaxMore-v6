...
export default function SilverCompetitionPage() {
  ...
  useEffect(() => {
    const fetchLeagueData = async () => {
      setLoading(true);
      try {
        const leagueData = await LeagueService.getLeague("silver") // Changed to silver
        ...
        {loading || isUserLoading ? (
          ...
          <>
            <div className="text-center mb-8">
              <Badge className="mb-4 bg-slate-100 text-slate-800">{league.name}</Badge> {/* Changed badge color */}
              <h1 className="text-4xl font-bold text-gray-900 mb-4">{league.name} Trading Competition</h1>
              <p className="text-xl text-gray-600">Advance from Bronze and compete for bigger prizes!</p> {/* Updated description */}
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-8 mb-8">
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center">
                    <Trophy className="h-5 w-5 mr-2 text-slate-600" /> {/* Changed icon color */}
                    Competition Details
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">$25,000</span> {/* Updated starting capital */}
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Max Participants:</span>
                    <span className="font-medium">500</span> {/* Updated max participants */}
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  {/* Prize pool details - updated for Silver */}
                  <div className="flex justify-between">
                    <span className="text-gray-600">1st Place:</span>
                    <span className="font-medium">$1,000 + Premium Access</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">Advanced Educational Resources</span>
                  </div>
                </CardContent>
              </Card>
            </div>
            ...
            <Card className="mt-8">
              <CardHeader>
                <CardTitle className="flex items-center">
                  ...
                  Current Participants: 123/500
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div className="bg-blue-600 h-2 rounded-full" style={{ width: "24.6%" }}></div> {/* Updated fill percentage */}
                </div>
                <p className="text-sm text-gray-600 mt-2">Competition starts when 250 participants join</p> {/* Updated text */}
              </CardContent>
            </Card>
          </>
        ) : (
