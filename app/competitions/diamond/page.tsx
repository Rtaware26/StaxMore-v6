...
export default function DiamondCompetitionPage() {
  ...
  const [competitionStatus, setCompetitionStatus] = useState<string>('unknown');
  const [userTimezone, setUserTimezone] = useState<string>('UTC');
  const [countdownText, setCountdownText] = useState<string>('loading...');

  // Define the total prize pool for this specific diamond competition page
  const MAX_COMPETITION_PARTICIPANTS = 100; // Fixed max participants for all competitions
  const TOTAL_DIAMOND_PRIZE_POOL = league ? league.entry_fee * MAX_COMPETITION_PARTICIPANTS : 0; // Dynamically calculate prize pool
  const DIAMOND_STARTING_CAPITAL = 100000; // From image
  ...
  useEffect(() => {
    ...
    const fetchCompetitionData = async () => {
      setLoading(true);
      try {
        const leagueData = await LeagueService.getLeague("diamond");
        ...
        {loading || isUserLoading ? (
          ...
          <>
            <div className="text-center mb-8">
              <Badge className="mb-4 bg-purple-500 text-purple-50">{league.name} - {league.competition_type?.toUpperCase()}</Badge>
              <h1 className="text-4xl font-bold text-foreground mb-4">{league.name} Trading Competition</h1>
              <p className="text-xl text-muted-foreground">Prove your mastery in the ultimate trading challenge!</p>
            </div>
              ...
              <Card className="card-modern">
                ...
                <CardContent className="space-y-4">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-purple-500">${league.entry_fee}</span>
                  </div>
                  <div className="flex justify-between">
              ...
              <Card className="card-modern">
                ...
                <CardContent className="space-y-4">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-primary">{formatCurrencyAbbreviated(TOTAL_DIAMOND_PRIZE_POOL)}</span>
                  </div>
                  <div className="flex justify-between">
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <span className="font-medium text-primary cursor-help">{formatCurrencyAbbreviated(TOTAL_DIAMOND_PRIZE_POOL * 0.40)}</span>
                        </TooltipTrigger>
                  ...
                  <div className="flex justify-between">
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <span className="font-medium text-primary cursor-help">{formatCurrencyAbbreviated(TOTAL_DIAMOND_PRIZE_POOL * 0.20)}</span>
                        </TooltipTrigger>
                        <TooltipContent className="p-2 bg-popover text-popover-foreground rounded-md shadow-lg">
                          20% of Prize Pool
                        </TooltipContent>
                  ...
                  <div className="flex justify-between">
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <span className="font-medium text-primary cursor-help">{formatCurrencyAbbreviated(TOTAL_DIAMOND_PRIZE_POOL * 0.10)}</span>
                        </TooltipTrigger>
                        <TooltipContent className="p-2 bg-popover text-popover-foreground rounded-md shadow-lg">
                          10% of Prize Pool
                        </TooltipContent>
                  ...
                  <div className="flex justify-between">
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <span className="font-medium text-primary cursor-help">{formatCurrencyAbbreviated(TOTAL_DIAMOND_PRIZE_POOL * 0.04)}</span>
                        </TooltipTrigger>
                        <TooltipContent className="p-2 bg-popover text-popover-foreground rounded-md shadow-lg">
                          4% of Prize Pool
                        </TooltipContent>
                  ...
                  <div className="flex justify-between">
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <span className="font-medium text-primary cursor-help">{formatCurrencyAbbreviated(TOTAL_DIAMOND_PRIZE_POOL * 0.02)}</span>
                        </TooltipTrigger>
                        <TooltipContent className="p-2 bg-popover text-popover-foreground rounded-md shadow-lg">
                          2% of Prize Pool
                        </TooltipContent>
                  ...
                  <div className="flex justify-between">
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <span className="font-medium text-primary cursor-help">{formatCurrencyAbbreviated(TOTAL_DIAMOND_PRIZE_POOL * 0.01)}</span>
                        </TooltipTrigger>
                        <TooltipContent className="p-2 bg-popover text-popover-foreground rounded-md shadow-lg">
            ...
            <Card className="mb-8 card-modern">
              ...
              <CardContent>
                <ul className="space-y-2 text-muted-foreground">
                  <li>• Start with ${DIAMOND_STARTING_CAPITAL} virtual capital</li>
                  <li>• Trade all available asset classes (Stocks, ETFs, Forex, Crypto, Commodities, Options)</li>
                  <li>• Options trading enabled</li>
                  <li>• Maximum position size: 5% of portfolio</li>
                  <li>• Real-time market data and pricing</li>
                  <li>• Rankings based on total portfolio value</li>
                  <li>• Must maintain minimum 1 trade per week</li>
                </ul>
              </CardContent>
            </Card>
            ...
            <div className="text-center">
              ...
              >
                {competitionStatus === 'registration-open' && `Join Competition (Closes ${countdownText})`}
                {competitionStatus === 'upcoming-registration' && `Registration Opens ${countdownText}`}
                {competitionStatus === 'registration-closed' && `Registration Closed (Starts ${countdownText})`}
                {competitionStatus === 'active' && `Competition Active (Ends ${countdownText})`}
                      ...
                      <tbody className="divide-y divide-border">
                        {leaderboard.map((portfolio, index) => (
                          <tr key={portfolio.user_id}>
                            ...
                            <td className={`px-4 py-2 whitespace-nowrap text-sm ${portfolio.total_equity - DIAMOND_STARTING_CAPITAL >= 0 ? "text-green-500" : "text-red-500"}`}>
                              {((portfolio.total_equity - DIAMOND_STARTING_CAPITAL) / DIAMOND_STARTING_CAPITAL * 100).toFixed(2)}%
                            </td>
