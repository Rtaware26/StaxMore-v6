...
export default function BronzeCompetitionPage() {
  ...
  // Define the total prize pool for this specific bronze competition page
  const MAX_COMPETITION_PARTICIPANTS = 100; // Fixed max participants for all competitions
  const BRONZE_STARTING_CAPITAL = 10000; // From image
    ...
    <div className="min-h-screen bg-background py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        {loading || isUserLoading ? (
          ...
          <>
            <div className="text-center mb-8">
              {league.competition_type && (
                <Badge className="mb-4 bg-secondary text-secondary-foreground">{league.name} - {league.competition_type.toUpperCase()}</Badge>
              )}
              <h1 className="text-4xl font-bold text-foreground mb-4">{league.name} Trading Competition</h1>
              <p className="text-xl text-muted-foreground">Perfect for beginners - Free entry with real prizes!</p>
            </div>
              ...
              <Card className="card-modern">
                ...
                <CardContent className="space-y-4">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-green-500">${league.entry_fee}</span>
                  </div>
                  <div className="flex justify-between">
            ...
            <Card className="mb-8 card-modern">
              ...
              <CardContent>
                <ul className="space-y-2 text-muted-foreground">
                  <li>• Start with ${BRONZE_STARTING_CAPITAL} virtual capital</li>
                      ...
                      <tbody className="divide-y divide-border">
                        {leaderboard.map((portfolio, index) => (
                          <tr key={portfolio.user_id}>
                            ...
                            <td className={`px-4 py-2 whitespace-nowrap text-sm ${portfolio.total_equity - BRONZE_STARTING_CAPITAL >= 0 ? "text-green-500" : "text-red-500"}`}>
                              {((portfolio.total_equity - BRONZE_STARTING_CAPITAL) / BRONZE_STARTING_CAPITAL * 100).toFixed(2)}%
                            </td>
          ...
          <div className="text-center py-8 text-muted-foreground">Failed to load competition details.</div>
        )}
      </div>
    </div>
  )
